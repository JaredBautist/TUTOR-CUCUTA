...
| GM-07 | WHEN the user activates the map's own-location control and grants permission, the system SHALL display the device's observed location, reported accuracy and observation state, then update it while tracking is active. | Controlled position events followed by a real-device check. |
...
| GM-09 | WHEN the student opens Mi Perfil and enables location, its existing map SHALL display the student's current device position. Exact observations SHALL stay in session memory and SHALL NOT silently replace the saved address or be published. | Own-position view and persistence/network inspection. |
...
| GM-13 | WHEN the user requests reduced motion, the system SHALL apply radius/position changes without decorative animation. Otherwise, marker movement SHALL interpolate only between received observations. | Reduced-motion and rapid-update tests. |
...
| GM-17 | WHEN the user presses Ver tutores, the system SHALL capture the visible origin and radius for that search. Later device observations SHALL NOT silently change that search area; the user's own position remains a separate marker. | Search-to-Results origin snapshot and subsequent position update. |
